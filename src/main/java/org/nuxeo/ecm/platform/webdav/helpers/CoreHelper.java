...
public class CoreHelper {
	...
	public static String getRealPath(WebDavRequestWrapper req) {
		...
		if (rPath == null) {
			...
			if (req.needGetParameterForCollectionNamming()) {
				// some crappy client will ignore the ? in the URL and append
				// the path
				// to the QueryString :)
				String decorator = req.getParameter(NuxeoWebDavServlet.GET_PARAMETER_DECORATOR);
				if (decorator != null) {
	...
	public static CoreSession getAssociatedCoreSession(WebDavRequestWrapper req) throws Exception {
		...
		if (session == null) {
			...
			if (repo == null) {
				throw new ClientException("Unable to get " + getRepositoryName(req) + " repository");
			}
			session = repo.open();
	...
	public static DocumentModel resolveTarget(WebDavRequestWrapper req) throws Exception {
		String docPath = getDocumentPath(req);

		return resolveTarget(req, docPath);
	}

	public static DocumentModel resolveTarget(WebDavRequestWrapper req, String docPath) throws Exception {
	 ...
	 * @return
	 */
	public static DocumentModel resolveVirtualPath(WebDavRequestWrapper req, String containerPath, List<String> subPath) throws ClientException {
		CoreSession session;
		try {
	 ...
	 * @return
	 */
	public static DocumentModel resolveVirtualPath(CoreSession session, String containerPath, List<String> subPath) throws ClientException {
		...
		DocumentRef parentRef = URLResolverCache.resolveUrlFromCache(session.getRepositoryName(), parentPath);
		if (parentRef == null) {
	...
	public static DocumentModel resolveVirtualChild(CoreSession session, DocumentModel container, String vPath) throws ClientException {
	...
	@Deprecated
	public static DocumentModel resolveVirtualLiefPath(WebDavRequestWrapper req, String docPath) throws ClientException {
		CoreSession session;
		try {
			session = getAssociatedCoreSession(req);
		} catch (Exception e) {
		...
		throw new ClientException("Unable to fetch document with path " + docPath);
	}
