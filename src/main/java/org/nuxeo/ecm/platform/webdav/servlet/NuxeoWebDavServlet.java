...
public class NuxeoWebDavServlet extends ExtensibleWebdavServlet {
	...
	@Override
	protected void doPropfind(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		...
		try {
			writeProperties(davResponse, requestedProperties, doc, depth, 0, davRequest);
		} catch (ClientException e) {
	...
	protected static void writeProperties(WebDavResponseWrapper davResponse, Map<String, List<String>> requestedProperties, DocumentModel doc, String maxDepth, int depth,
			WebDavRequestWrapper davRequest) throws ClientException {
		...
		for (String uri : requestedProperties.keySet()) {
			if (uri.equals("*")) {
				...
				for (String schemaURI : allProps.keySet()) {
					...
					davResponse.addProperties(getRessourceURL(doc, davRequest, depth), schemaURI, allProps.get(schemaURI));
				}
				...
				for (String schemaURI : allPropNames.keySet()) {
					...
					if (!(depth == 0 && davRequest.skipLevel0ForListing())) {
						davResponse.addProperties(getRessourceURL(doc, davRequest, depth), schemaURI, emptyProps);
					}
				}

			} else {
				List<String> fieldNames = requestedProperties.get(uri);
				Map<String, String> props = MappingHelper.getMapperForURI(uri).getDavProperties(doc, fieldNames);
				if (!(depth == 0 && davRequest.skipLevel0ForListing())) {
					davResponse.addProperties(getRessourceURL(doc, davRequest, depth), uri, props);
				}
			}
		}
		...
		if (doc.isFolder()) {
			...
			try {
				for (DocumentModel child : session.getChildren(doc.getRef())) {
					writeProperties(davResponse, requestedProperties, child, maxDepth, depth + 1, davRequest);
				}
			} catch (ClientException e) {
				log.error("Error while writing properties in XML Body : " + e.getMessage(), e);
				return;
			} catch (Throwable t) {
				log.error("Error while writing properties in XML Body : " + t.getMessage(), t);
				return;
			}
		}
	}
	...
	@Override
	protected void doMove(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doMove");
		doCopyOrMove(davRequest, davResponse, false);
	}

	@Override
	protected void doLock(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doLock");
		try {
...
//				target.setPropertyValue("dc:title", fileName);
			...
			int result = lockableDoc.lock(davRequest.getUserPrincipal().getName());
	...
	@Override
	protected void doUnlock(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doUnlock");
		try {
			DavLockInfo lockInfo = davRequest.extractLockInfo();
			...
			LockableDocument lockableDoc = target.getAdapter(LockableDocument.class);
			int result = lockableDoc.unlock(davRequest.getUserPrincipal().getName());
	...
	@Override
	protected void doMkcol(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doMkcol");
		...
		log.debug("Creating Collection " + colName + " at path " + containerPath);
		...
		if (typeAdapter != null) {
			List<String> allowedTypes = new ArrayList<String>(Arrays.asList(typeAdapter.getAllowedSubTypes()));

			if (!allowedTypes.contains(targetType)) {
				log.error("Can not create type " + targetType + " as child of a " + parentType);
				davResponse.setStatus(WebDavConst.SC_FORBIDDEN);
				return;
			}
		}
		...
		try {
			...
			DocumentModel collec = session.createDocumentModel(parent.getPathAsString(), colId, targetType);
			...
			String virtualURL = '/' + collec.getRepositoryName() + parent.getPathAsString() + '/' + colName;
			String virtualURL2;
			if (containerPath.endsWith("/")) {
				virtualURL2 = '/' + collec.getRepositoryName() + containerPath + colName;
			} else {
				virtualURL2 = '/' + collec.getRepositoryName() + containerPath + '/' + colName;
			}
	...
	@Override
	protected void doCopy(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doCopy");
		doCopyOrMove(davRequest, davResponse, true);
	}

	protected void doCopyOrMove(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse, boolean copy) {
		log.debug("doCopy");
		try {
			...
			List<String> targetInfo = getPathFromFullURL(davRequest, destination);
			...
			DocumentModel targetDoc = CoreHelper.resolveTarget(davRequest, targetPath);
			...
			if (targetAlreadyExists) {
				...
				String targetContainerPath = targetPath.replace('/' + targetDocumentName, "");

				DocumentModel targetContainer = CoreHelper.resolveTarget(davRequest, targetContainerPath);

				if (targetContainer != null) {
					targetContainerRef = targetContainer.getRef();
				} else {
					log.error("destination container does not exist:" + targetContainerPath);
					davResponse.setStatus(WebDavConst.SC_CONFLICT);
					return;
				}
				...
				if (targetDocumentName.contains("?" + GET_PARAMETER_DECORATOR)) {
					targetDocumentName = targetDocumentName.split("\\?" + GET_PARAMETER_DECORATOR + "=")[1];
			...
			if (copy) {
				session.copy(source.getRef(), targetContainerRef, targetDocumentName);
				session.save();
			} else {
				...
				if (sourceContainer.getRef().equals(targetContainerRef)) {
					...
					// real move
					session.move(source.getRef(), targetContainerRef, targetDocumentName);
					session.save();
					URLResolverCache.removeFromCache(source);
				}
			}
	...
	@Override
	protected void doProppatch(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doProppatch");
		davResponse.setUnimplemented();
	}

	@Override
	protected void doPut(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		...
		try {
			...
			try {
				...
				log.error("Unable to read request inputStream : " + e.getMessage());
				davResponse.setStatus(WebDavConst.SC_INTERNAL_SERVER_ERROR);
				return;
			}
			...
			try {
				...
				if (davRequest.needGetParameterForCollectionNamming()) {
					...
					DocumentModel container = CoreHelper.resolveTarget(davRequest, containerPath);
					if (container == null) {
						davResponse.setStatus(WebDavConst.SC_INTERNAL_SERVER_ERROR);
						return;
					}
				...
				fm.createDocumentFromBlob(session, thefile, containerPath, overwrite, fileName);
				session.save();
			} catch (Exception e) {
				log.error("Error when creating the document via FileManager: " + e.getMessage(), e);
				davResponse.setStatus(WebDavConst.SC_INTERNAL_SERVER_ERROR);
				return;
			}
	...
	@Override
	protected void doPost(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doPost");
		davResponse.setUnimplemented();
	}

	@Override
	protected void doGet(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doGet");
		doGetOrHead(davRequest, davResponse, true);
	}

	@Override
	protected void doHead(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doHead");
		doGetOrHead(davRequest, davResponse, false);
	}

	protected void doGetOrHead(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse, boolean includeBody) {
		log.debug("doGetOrHead");

		if (!includeBody) {
			NoBodyResponse response = new NoBodyResponse((HttpServletResponse) davResponse.getResponse());
	...
	@Override
	protected void doDelete(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doDelete");
		try {
			DocumentModel target = CoreHelper.resolveTarget(davRequest);
			CoreSession session = CoreHelper.getAssociatedCoreSession(davRequest);
	...
	@Override
	protected void doOptions(WebDavRequestWrapper davRequest, WebDavResponseWrapper davResponse) {
		log.debug("doOptions");
		// Dav Compatibility
		davResponse.addHeader(WebDavConst.HEADER_DAV, "1,2");

		// XXX : sould respond according to resource !
		davResponse.addHeader("Allow", "GET, PUT, POST, HEAD, PROPFIND, PROPPATCH, OPTIONS, LOCK, UNLOCK, MOVE, COPY, MKCOL, DELETE");
	...
	private static String getRessourceURL(DocumentModel doc, WebDavRequestWrapper request, int depth) throws ClientException {
		if (depth > 0) {
	...
	private static String virtualHostURL(WebDavRequestWrapper request, String url) {
		...
		if (nuxeoVH != null) {
			if (nuxeoVH.endsWith("/")) {
				virtualHostedBaseURL = nuxeoVH.substring(0, nuxeoVH.length() - 1) + PATTERN;
			} else {
				virtualHostedBaseURL = nuxeoVH + PATTERN;
			}
		} else {
			virtualHostedBaseURL = request.getScheme() + "://" + forwardedHost + PATTERN;
		}
	...
	private static String getResourceURL(DocumentModel doc, WebDavRequestWrapper request) throws ClientException {
		...
		if (request.needVirtualPathForLief() && !doc.isFolder()) {
			...
			String truncatedPath = resourceURL.replace(doc.getName(), realFileName);
			URLResolverCache.addToCache(truncatedPath, doc);

			resourceURL = resourceURL + '/' + MSDavConst.MS_DAV_URL_DELIMITER + '/' + realFileName;
		...
		if (doc.isFolder() && request.needGetParameterForCollectionNamming()) {
			String truncatedPath = resourceURL.replace(doc.getName(), doc.getTitle());
			URLResolverCache.addToCache(truncatedPath, doc);
			resourceURL = resourceURL + '?' + GET_PARAMETER_DECORATOR + "=/" + doc.getTitle();
		}

		return resourceURL;
	}

	private static List<String> getPathFromFullURL(WebDavRequestWrapper request, String url) {
