...
public class NuxeoWebDavServlet extends ExtensibleWebdavServlet {
        ...
        for (String uri : requestedProperties.keySet()) {
            if (uri.equals("*")) {
                ...
                for (String schemaURI : allPropNames.keySet()) {
                    ...
                    /*
                    if (davRequest.skipLevel0ForListing())
                    {
                        boolean canSkip=false;
                        // check if we can skip Level0 Listing
                        if (depth==0 && !maxDepth.equals("0") && doc.isFolder())
                        {
    ...
    protected void doPut(WebDavRequestWrapper davRequest,
        ...
        String path = CoreHelper.getDocumentPath(davRequest); // davRequest.getRelativePath();
        String[] pathParts = path.split("/");
